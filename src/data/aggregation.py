...
def _downcast_numeric_columns(
    df: pd.DataFrame, *, key_cols: tuple[str, ...] = ("SK_ID_CURR", "SK_ID_BUREAU")
) -> pd.DataFrame:
    if df.empty:
        return df
    out = df
...
def _extended_agg_dict(num_cols: list[str]) -> dict[str, list[str]]:
    """Standard numeric aggregations including distribution shape statistics."""
    stats = ["mean", "max", "min", "sum", "count", "std", "median", "skew"]
    return {c: stats for c in num_cols}
...
def aggregate_bureau_balance(bureau_balance: pd.DataFrame) -> pd.DataFrame:
    ...
    if num_cols:
        agg_dict = _extended_agg_dict(num_cols)
...
def aggregate_bureau(bureau: pd.DataFrame, bb_per_bureau: pd.DataFrame | None) -> pd.DataFrame:
    ...
    agg_dict = _extended_agg_dict(num_for_agg)
...
def aggregate_installments(inst: pd.DataFrame) -> pd.DataFrame:
    ...
    if "INST_IS_LATE" in ins.columns:
        g = g.merge(
            ins.groupby("SK_ID_CURR")["INST_IS_LATE"].mean().rename("INST_LATE_PAYMENT_RATE").reset_index(),
            on="SK_ID_CURR",
            how="left",
        )
    ...
    if "_REC_W" in ins.columns and "INST_PAYMENT_DIFF_POS" in ins.columns:
        ...
        g = g.merge(
            (num / den.replace(0, np.nan)).rename("INST_RECENCY_WMEAN_LATE_DAYS").reset_index(),
            on="SK_ID_CURR",
            how="left",
        )
...
def aggregate_pos_cash(pos: pd.DataFrame) -> pd.DataFrame:
    ...
    if "POS_DPD_POS" in p.columns:
        g = g.merge(
            p.groupby("SK_ID_CURR")["POS_DPD_POS"].mean().rename("POS_DELINQ_FREQ").reset_index(),
            on="SK_ID_CURR",
            how="left",
        )
    if "MONTHS_BALANCE" in p.columns:
        ...
        if "SK_DPD" in p.columns:
            ...
            g = g.merge(
                (num / den.replace(0, np.nan)).rename("POS_RECENCY_WMEAN_DPD").reset_index(),
                on="SK_ID_CURR",
                how="left",
            )
    ...
    if "SK_DPD" in p.columns:
        g = g.merge(
            p.groupby("SK_ID_CURR")["SK_DPD"].max().rename("POS_MAX_SK_DPD").reset_index(), on="SK_ID_CURR", how="left"
        )
        g = g.merge(
            ((p.groupby("SK_ID_CURR")["SK_DPD"].max() > 0).astype(np.float32)).rename("POS_ANY_HIST_DELINQ").reset_index(),
            on="SK_ID_CURR",
            how="left",
        )
    return g
...
def aggregate_credit_card(cc: pd.DataFrame) -> pd.DataFrame:
    ...
    if "CC_HIGH_UTIL" in c.columns:
        g = g.merge(
            c.groupby("SK_ID_CURR")["CC_HIGH_UTIL"].mean().rename("CC_HIGH_UTILIZATION_FREQ").reset_index(),
            on="SK_ID_CURR",
            how="left",
        )
    if "CC_UTILIZATION" in c.columns:
    ...
    if "CC_DPD_POS" in c.columns:
        g = g.merge(
            c.groupby("SK_ID_CURR")["CC_DPD_POS"].mean().rename("CC_DELINQ_FREQ").reset_index(),
            on="SK_ID_CURR",
            how="left",
        )
    ...
    if "MONTHS_BALANCE" in c.columns and "CC_UTILIZATION" in c.columns:
        ...
        g = g.merge(
            (num / den.replace(0, np.nan)).rename("CC_RECENCY_WMEAN_UTIL").reset_index(), on="SK_ID_CURR", how="left"
        )
